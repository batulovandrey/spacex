...
class NoSSLv3SocketFactory(private val delegate: SSLSocketFactory) : SSLSocketFactory() {
    ...
    private fun makeSocketSafe(socket: Socket): Socket {
        var resultSocket = socket
        if (resultSocket is SSLSocket) {
            resultSocket = NoSSLv3SSLSocket(resultSocket)
        }
        return resultSocket
    }
    ...
    private inner class NoSSLv3SSLSocket internal constructor(delegate: SSLSocket) : DelegateSSLSocket(delegate) {

        override fun setEnabledProtocols(protocols: Array<String>?) {
            var tempProtocols = protocols
            if (tempProtocols != null && tempProtocols.size == 1 && "SSLv3" == tempProtocols[0]) {

                val enabledProtocols = ArrayList<String>()
                enabledProtocols.addAll(delegate.enabledProtocols)
                if (enabledProtocols.size > 1) {
                    enabledProtocols.remove("SSLv3")
                    println("Removed SSLv3 from enabled protocols")
                } else {
                    println("SSL stuck with protocol available for " + enabledProtocols.toString())
                }
                tempProtocols = enabledProtocols.toTypedArray()
            }

            super.setEnabledProtocols(tempProtocols)
        }
    }
